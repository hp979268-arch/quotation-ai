...
# Load dual images list so we skip them
v2_files = [f for f in os.listdir(IMAGE_DIR) if f.endswith('_v2.png')]
v2_base_names = {f.replace('_v2.png', '') for f in v2_files}

# Get list of products to process (trim+valve ones, excluding v2)
...
for item in items:
    ...
    if 'kohler' in item.get('brand','').lower() and sc in tv_codes:
        # Skip if it's one of the dual images
        if sc in v2_base_names:
            continue
...
print(f"Processing {len(to_process)} products (excluding dual images)...")
