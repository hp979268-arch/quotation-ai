...
# Gather info from existing entries
# 30006 = 1.0 mtr, 30007 = 1.5 mtr
# We need 7 joint entries

# Get one 30006 item to use as template (for brand, category etc.)
template = next((i for i in items if i.get('search_code','').startswith('30006 CP')), None)

# Build 7 new joint entries
finishes = {
...
for finish, info in finishes.items():
    ...
    entry = {
        "text": f"{new_code} - {desc}",
        "name": f"{new_code} - Extendible Shower Hose (SS) - {info['label']}",
        "price": info['price_1m'],  # base price for 1.0 mtr
        ...
        "images": [f"/static/images/Aquant/30006-30007 {finish}.png?v=8"],
        "brand": "Aquant",
